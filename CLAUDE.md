...
**ドキュメントはプロダクトの実装と常に一致させる。実装されていない機能をドキュメントに書いてはいけない。**

### ルール

| # | ルール |
|---|-------|
| **D1** | `docs/` のドキュメントに記載する機能は、**実際に動作するものだけ**に限定する |
| **D2** | 未実装・未接続の機能は「未実装機能」セクションに明記する。動くように見せかけて書かない |
| **D3** | 実装したら対応するドキュメントを同じ PR で更新する |
| **D4** | 実装を削除・変更したら対応するドキュメントも同じ PR で更新する |
| **D5** | `docs/voice-operations.md` は `content.js` の `onResult` ハンドラが実際に処理するアクションのみ記載する |

### 背景（なぜこのルールが必要か）

`docs/voice-operations.md` に「レコード作成」「レコード更新」「情報集計・要約」「undo」「停止」が実装済みかのように記載されていたが、
`content.js` にはこれらの処理ブランチが存在せず、発話しても「認識: xxx」と表示されるだけだった。
ユーザーが動くと思って試しても何も起きない状態が生まれていた。

### 確認方法

`docs/voice-operations.md` を変更する前に、`content.js` の `onResult` ハンドラで
そのアクションが実際に処理されているか確認する:

```javascript
// content.js の onResult — 処理されるアクションのみドキュメントに書く
if (intent.action === 'navigate') { ... }      // ✅ 実装済み → 記載OK
else if (intent.action === 'back')  { ... }    // ✅ 実装済み → 記載OK
else if (intent.action === 'search') { ... }   // ✅ 実装済み → 記載OK
else if (intent.action === 'select') { ... }   // ✅ 実装済み → 記載OK
else { /* 認識表示のみ */ }                    // ❌ stop/undo/help/confirm は未実装 → 「未実装」と明記
```
