...
|---|---|
| **1件** | そのレコードを直接開く |
| **2〜5件** | `/lightning/search?searchInput={keyword}` へ遷移（候補確認） |
| **6件以上** | `/lightning/search?searchInput={keyword}` へ遷移（絞り込みを促す） |
| **0件** | 「見つかりません。グローバル検索に移動します」と表示し、約2秒後に `/lightning/search?searchInput={keyword}` へ自動遷移 |

> **曖昧検索**: 「株式会社」「かぶしきがいしゃ」などの法人格表記ゆれに対応するため、
> 複数バリアントで SOSL 検索を試みます（soslFuzzy）。それでも0件の場合はグローバル検索へ自動遷移します。
