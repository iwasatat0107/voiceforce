...
> **処理**: ruleEngine（正規表現）で実行します。**以下のパターンのみ対応。**
> パターン外の発話（例：「先週作った取引先を探して」）は現在未対応です。

### 対応している発話パターン

| 発話形式 | 発話例 | 動作 |
|---|---|---|
| `[キーワード]の[オブジェクト]を開いて` | 「田中商事の商談を開いて」 | 田中商事の商談を検索 |
| `[キーワード]の[オブジェクト]を見せて` | 「山田太郎の取引先責任者を見せて」 | 山田太郎を Contact 検索 |
| `[オブジェクト]の[キーワード]を表示して` | 「取引先のABC株式会社を表示して」 | ABC株式会社を Account 検索 |
| `[オブジェクト]で[キーワード]を検索して` | 「商談で田中を検索して」 | 田中を Opportunity 検索 |
| `[キーワード]を表示して` | 「ABC株式会社を表示して」 | ABC株式会社を Account 検索 |
| `[キーワード]を見せて` | 「田中商事を見せて」 | 田中商事を Account 検索 |

> **注意**: `[キーワード]を表示して` / `[キーワード]を見せて` 形式はオブジェクトを指定しない場合、
> **取引先（Account）のみ**を検索します。
...
| 0件 | 「見つかりませんでした」と表示 |
| 1件 | そのレコードページへ自動遷移 |
| 2〜5件 | 候補リストが表示される → 「1番」〜「5番」で選択 |
| 6件以上 | 「件数が多すぎます。条件を絞ってください」と表示 |
