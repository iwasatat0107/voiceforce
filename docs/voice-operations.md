...
> **処理**: ruleEngine（正規表現）でキーワードを抽出し、Salesforce のグローバル検索ページへ遷移します。
> 検索結果の表示・絞り込みは Salesforce の画面で行ってください。

### 対応している発話パターン

| 発話形式 | 発話例 | 動作 |
|---|---|---|
| `[キーワード]の[オブジェクト]を開いて` | 「田中商事の商談を開いて」 | 「田中商事」でグローバル検索 |
| `[キーワード]の[オブジェクト]を見せて` | 「山田太郎の取引先責任者を見せて」 | 「山田太郎」でグローバル検索 |
| `[オブジェクト]の[キーワード]を表示して` | 「取引先のABC株式会社を表示して」 | 「ABC株式会社」でグローバル検索 |
| `[オブジェクト]で[キーワード]を検索して` | 「商談で田中を検索して」 | 「田中」でグローバル検索 |
| `[キーワード]を表示して` | 「ABC株式会社を表示して」 | 「ABC株式会社」でグローバル検索 |
| `[キーワード]を見せて` | 「田中商事を見せて」 | 「田中商事」でグローバル検索 |

抽出したキーワードを `/lightning/search?searchInput={keyword}` に渡してナビゲートします。
OAuth トークンや API 呼び出しは不要です。
